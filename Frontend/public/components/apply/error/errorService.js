MyApp.service('ErrorService',['$http','keybusinessService',function ($http,keybusinessService) {

    var self= this

    //API
    this.get_error_analysis_error_and_exception_trend_chart = env_config.API.Error.GET_ERROR_ANALYSIS_ERROR_AND_EXCEPTION_TREND_CHART
    this.get_error_analysis_error_rate_pie = env_config.API.Error.GET_ERROR_ANALYSIS_ERROR_RATE_PIE

    this.get_error_analysis_exception_rate_pie= env_config.API.Error.GET_ERROR_ANALYSIS_EXCEPTION_RATE_PIE
    //错误列表
    this.transaction_list = env_config.API.Error.GET_ERROR_ANALYSIS_ERROR_LIST;
    //异常列表
    this.get_error_analysis_exception_list = env_config.API.Error.GET_ERROR_ANALYSIS_EXCEPTION_LIST
    ...
    //错误|异常列表
    this.errorList = function (opt,callback) {
         var URL =self.transaction_list
        $('#errorList').table(function () {
            ...
            return {
                ...
                loop: function (item, tr) {
                    if (item.key == 'event') {
                        return '<td class="text-ellipsis" style="text-align: left"><a target="_blank" href="#/errorDetail/' + encodeURIComponent(encodeURIComponent(item.value)) +'/'+ opt.app_id+'/'+ tr.message +'/'+ tr.start_time +'/'+ tr.end_time + '" class="business-details">' + item.value + '</a></td>';
                    }
                }
            }
        });
    }
    this.abnormalList = function (opt,callback) {
        var URL =self.get_error_analysis_exception_list
        $('#errorList').table(function () {
            var data = {
                app_id: opt.app_id,
                start_time: opt.start_time,
            ...
            return {
                ...
                loop: function (item, tr) {
                    if (item.key == 'event') {
                        return '<td class="text-ellipsis" style="text-align: left"><a target="_blank" href="#/abnormalDetail/' + encodeURIComponent(encodeURIComponent(item.value)) +'/'+ opt.app_id+ '/'+ encodeURIComponent(encodeURIComponent(tr.message))  +'/'+ tr.start_time + '/'+ tr.end_time+'" class="business-details">' + item.value + '</a></td>';
                    }
                }
            }
        });
