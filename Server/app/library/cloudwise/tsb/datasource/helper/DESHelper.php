...
class DESHelper
{
    ...
    private static $version = DESHelper::VERSION_V_1;
    ...
    private static $aesIv = "1234567812345678";

    static public function instance($version = DESHelper::VERSION_V_1)
    {
    ...
    public function __construct($version)
    {
        ...
        $des_key = ConfigService::instance()->getConfig('data.des.des_key');
        $des_iv = ConfigService::instance()->getConfig('data.des.des_iv');
    ...
    private function encode_v2($str)
    {
        ...
        $ciphertext = str_replace(
            array_keys($this->aReplace), array_values($this->aReplace), base64_encode($ciphertext));
    ...
    public function encodeDownloadToken($account_id)
    {
        return base_convert($account_id + 100000, 10, 32);
    }
     ...
     * @return string
     */
    public function decodeDownloadToken($token)
    {
        return (string)(base_convert($token, 32, 10) - 100000);
    }

    //urldecode 加号转化为空格的问题
    public function url_decode($str)
    {
        $str = str_replace(array_keys($this->aReplace), array_values($this->aReplace), $str);
        $str = urldecode($str);
        $str = str_replace(array_keys($this->aReplace), array_values($this->aReplace), $str);
        $str = urldecode($str);
        $str = str_replace(array_values($this->aReplace), array_keys($this->aReplace), $str);
     ...
     * @return string
     */
    public function AesDecode($secretKey, $data)
    {
        $text = $secretKey;
        $key = md5($text);  //key的长度必须16，32位,这里直接MD5一个长度为32位的key
        $Decode = mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $key, base64_decode($data), MCRYPT_MODE_CBC, self::$aesIv);
     ...
     * @return string
     */
    public function AesEncode($secretKey, $data)
    {
        $text = $secretKey;
        $key = md5($text);  //key的长度必须16，32位,这里直接MD5一个长度为32位的key
        $crypttext = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $data, MCRYPT_MODE_CBC, self::$aesIv);
